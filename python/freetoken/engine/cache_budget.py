...
def expert_bytes_per_slot(sources: dict[str, "list[torch.Tensor]"]) -> int:
    ...
    # Mixed GGUF levels can change packed row width by layer. The shared GPU slot pool
    # uses the widest layer in each bank, while compact host sources retain native sizes.
    return sum(
