...
def _resolve_auto_attention_backend(required: frozenset[AttnType]) -> str:
    ...
    if AttnType.QSA_TOKEN in required:
...
def _adjust_config(config: EngineConfig):
    ...
    # EngineConfig always carries this field.  A few internal callers intentionally use
    # duck-typed config views containing only the subsystem they exercise; for those the
    # public default is the correct value rather than an unrelated AttributeError.
    if getattr(config, "qsa_score_workspace_mib", EngineConfig.qsa_score_workspace_mib) <= 0:
    ...
    if getattr(config, "moe_collect_stats", False) and config.max_running_req != 1:
    ...
    if (
        required_attn_types & {AttnType.BSA, AttnType.QSA, AttnType.QSA_TOKEN}
