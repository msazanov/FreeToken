...
class KVCacheGroupSpec:
    ...
    # Token-resolution QSA compatibility metadata.
...
class ModelConfig:
    ...
    # Routed-expert bank ggml types for ``expert_quant == "gguf"``, as two per-layer
    # tuples ``(gate_up_types, down_types)``. Unlike other tags, which name a *format*
    ...
    #
    # The GPU pool reserves the maximum expert stride per bank; compact host layers are
    # copied into each slot prefix and the kernel dispatch selects the layer's type.
    ...
    # The GGUF/Turing compatibility path owns request-local state outside radix snapshots.
            ...
            if isinstance(group, FullAttentionGroupConfig):
                ...
                specs.append(
                    KVCacheGroupSpec(
                        ...
                        index_ratio=group.index_compress_ratio,
                        attn_type=AttnType.QSA_TOKEN,
                    )
                )
