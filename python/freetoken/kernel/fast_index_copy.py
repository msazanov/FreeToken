...
def fast_index_copy_multi_jit(
    ...
    *,
    ...
    ``feat_bytes`` is the number of native source bytes copied per row.
    ``dst_stride_bytes`` is the fixed slot-cache row stride and defaults to
    ``feat_bytes``. Keeping them separate lets compact mixed-quant GGUF layers share a
    max-width destination pool without reading source padding that does not exist.
    """
    if _skip_fast_index_copy_enabled():
        return
