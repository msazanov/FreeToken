...
## 2026-09-04 — Gemma cold-switch timing

Starting from a warm LFM2.5 backend, a real `gemma-4-e2b` request was sent
through the public arbiter. The lease was granted immediately and the daemon
accepted `POST /engine/start` at `23:08:43.576`; arbiter readiness was committed
at `23:10:37.355`, after 441 probes and `113777.7 ms`. The final completion
returned at `23:10:38.209`, so generation itself was sub-second after readiness.

This isolates the observed cold-start delay to Gemma engine startup/readiness
(model load, GPU admission and warmup), not queue waiting or first-token
generation. The daemon's captured engine log ring is now exposed through the
local UI for the next run. No benchmark artifact was generated by this timing
smoke.
