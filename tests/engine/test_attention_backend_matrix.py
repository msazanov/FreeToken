...
def _model_config(kind):
    ...
    if kind == "full":
        ...
        # GGUF/Turing compatibility path: request-local token-resolution QSA state.
        mc.has_linear_attention = True
        mc.requires_naive_cache = True
        mc.supports_cuda_graph = False
        specs = (_spec("qsa", AttnType.QSA_TOKEN, index_head_dim=128, index_ratio=4),)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("qsa_token", "qsa"),  # GGUF token-resolution compatibility backend
    ],
)
...
def test_auto_token_qsa_forces_safe_cache_and_disables_cuda_graphs(monkeypatch):
    from freetoken.engine.engine import _adjust_config

    _patch_env(monkeypatch)
    config = _config("qsa_token", attention_backend="auto")
    _adjust_config(config)
