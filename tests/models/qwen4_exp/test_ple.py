...
def test_track_snapshot_equals_a_prefill_stopped_at_the_boundary():
    ...
    # The full [CHUNK_SIZE + tail] and truncated [CHUNK_SIZE] CPU GEMMs may choose
    # different BLAS reduction orders.  The donated state is the same computation at
    # the boundary, but need not be bit-identical across those matrix shapes.
    torch.testing.assert_close(got, stopped[live], rtol=1e-5, atol=1e-6)
